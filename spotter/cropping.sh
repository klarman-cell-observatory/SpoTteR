...
# Run cropping script
RScript "$(echo $sp/cropping_spotter.R)" "$(echo "${array}_${cn}_${cde}")" $batch "$(echo "$sm")" "$(echo "$2")"
...
# crop
convert "$(echo $img)" -crop $(echo $width"x"$height+$xa+$ya) "$(echo "$array""$cn"_"$cde"_HE_crop.jpg)"
