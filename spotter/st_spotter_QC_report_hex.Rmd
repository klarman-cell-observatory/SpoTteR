...
read.pipe.log = function(pipe_log_file){
  ...
  # reads in log file
  # pipe_log_file = paste0(pipeline_logs_folder, "/", array, "_", cn,"_",cde, "_log.txt")
  ...
  merged.file = cbind(
    ...
    sapply(strsplit(sapply(strsplit(grep(pattern = "INFO:STPipeline:Trimming stats reverse: you just lost about ", x = row.names(log_file), value =T), " "), "[[", 8), "%"),"[[",1),
    ...
    sapply(strsplit(grep(pattern = "INFO:STPipeline:Number of unique transcripts present", x = row.names(log_file), value =T), ": "),"[[",2),
...
# qc_plot_folder = "/Users/svickovi/Library/Mobile Documents/com~apple~CloudDocs/Desktop/SpoTter/RA/output/QC_plots"
# full_names = "2018Diamond_CNO-254_E2"
...
df=read.pipe.log(paste0(pipeline_logs_folder, "/", array, "_", cn,"_",cde, "_log.txt"))
kable(df) %>%
...
## This renders the QC plot. Plot shows: 
    --- HE Tissue Image
    --- Heatmap of log2(raw expression) associated with all 1933 ST barcodes
...
## This renders the Interesting Genes QC plot. User defines interesting genes during SpoTter processing. Plot shows: 
    --- Heatmap of log2(raw expression) associated with 5 interesting genes in your ST experiment
