...
# mogrify -path "$(echo "$sm")" -resize 1% -quality 100%% $img_file_nm # to run locally
...
# run cropping script
bash "$(echo $sp/cropping.sh)" "$(echo ${ABSPATH%/*}/processing/${img_file_nm/.jpg/}_inferred_points.tsv)"

# run spotter QC R script on spotter output from previous step
RScript "$(echo $sp/st_spotter_QC_RUN.R)" "$(echo "$st")" $batch "$(echo "$sm")" "$(echo "$sp")"
rm "$(echo $sm/"$array""$cn"_"$cde"_HE.jpg)"

# trim pdf QC reports # in QC_plots dir already
cd "$(dirname "$sm")/output/QC_plots"
convert -density 300 ST_Report_Interesting_Genes_"$array"_"$cn"_"$cde".pdf -shave 10x450 -size 150%x150% -gravity East -chop 750x0 -gravity west -chop 450x0 ST_Report_Interesting_Genes_"$array"_"$cn"_"$cde".pdf
convert -density 300  ST_Report_Interesting_Genes_"$array"_"$cn"_"$cde".pdf -gravity West -chop 50x0 -gravity South -chop 0x150 -gravity North -chop 0x100 -gravity East -chop 500x0 ST_Report_Interesting_Genes_"$array"_"$cn"_"$cde".pdf
...
cp "$sp"/st_spotter_QC_report.Rmd ./
mv st_spotter_QC_report.Rmd "$rpt_name"
