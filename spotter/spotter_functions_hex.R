...
find_candidates = function(sub_data_table,n_nn,alpha) {
    dist_threshold = 0.01 # 3% relative error in the distance
    phi_threshold = 0.04 # 3%*pi absolute error in the angle
...
create_grid=function(angles,length,origin, nrows, ncols){
  ...
  if ((nrows > ncols) & (nrows %% 2 !=0 | ncols %% 2 !=0)) {
    #print("da")
    if (nrows %% 2 !=0){
      nrowst = nrows + 1
      ct1 = "da1"
  ...
  for (i in 0:nrowst){
    ...
    #print(j)
    if ((j%%2 != 0)){
      x = c(x, rep(i, ncolst/2))
      y = c(y, seq(2, ncolst, by = 2))
      #print("bok")
    } else {
      #print("ajde")
      x = c(x, rep(i, ncolst/2))
      y = c(y, seq(1, ncolst, by = 2))
    }
  }
  ...
  # if (((ct1 == "da1") & (ct2 == "da2")) | ((ct3 == "da3") & (ct4 == "da4"))){
  #   df = df[!df$x == max(x),]
  #   df = df[!df$y == max(y),]
  # } else if ((ct1 == "da1") | (ct3 == "da3")) df = df[!df$y == max(y),] else if ((ct2 == "da2") | (ct4 == "da4")) df = df[!df$x == max(x),]
  ...
  grid <- as.data.table(t(basis %*% rbind(xt, yt) + origin))
  setnames(grid, names(grid),c("x","y"))
  #plot(grid$x, grid$y)
...
remove_worst_fitting_points = function (pts, f_chi, relative_share_to_remove=0.001) {
    better_pts = pts
...
process_HE=function(image_file,pink=FALSE,bubble=TRUE){
    #-------LOAD DATA---------#
    #image_file = "/Users/svickovi/Library/Mobile Documents/com~apple~CloudDocs/Desktop/SpoTter/RA/processing/2018Diamond_CNO-254_RA.C2-small.jpg"
    ...
    HE2[,,,1][HE2[,,,1]<cutoff]=min(HE2[,,,1][HE2[,,,1]!=1])
    p1=ggplot(as.data.frame(HE2),aes(x,y))+ggpl_settings+ggtitle("Removed non-grid features")
    p1
    
    #detect blobs (grid points)
    #ib=isoblur(crop.borders(HE2,nPix = 5), 3.5)
    ib=isoblur(crop.borders(HE2,nPix = 5), 3)
    Hdet <- with(imhessian(ib),(xx+yy))
    hc.clean=Hdet ; hc.clean[dilate_square(hc.clean, 4) != hc.clean] <- 0
    ...
    n_nn = 16 # number of nearest neighbours to find
    ...
    #min_number_of_candidates = 5
    min_number_of_candidates = 1
    ...
    #bulid feature mask
    start_coords=which(HE3[,,,1] > 0.1,arr.ind = TRUE)[1,]
    ...
    HE3[,,,1][HE3[,,,1]>mean_bg-min(sd_bg/5,0.05)]=1
    HE3[,,,1][HE3[,,,1]<=mean_bg-min(sd_bg/5,0.05)]=0
    
    #plot(HE3)
    
    
    HE_msk=(msk|HE3)|bubble_mask
    HE4=isoblur(HE_msk,1)
    HE4[,,,1][HE4[,,,1]>0.99|HE_msk[,,,1]==1]=1
    HE5=isoblur(HE4,1)
    ...
    dy_fitter=sbgm(good_pts, seq(5.8,6.1,0.1), seq(0,7,1), 0)
    dx_fitter=sbgm(good_pts, seq(5.8,6.0,0.1), seq(0,7,1), pi/2)
    
    rough_grid=create_grid(angles = c(0,90),length = c(dx_fitter$par[1],dy_fitter$par[1]),origin = c(dx_fitter$par[2]-1*dx_fitter$par[1],dy_fitter$par[2]-1*dy_fitter$par[1]),ncols = 63,nrows = 65)
    ...
    #points(create_grid(angles = c(0,90),length = c(dx_fitter$par[1],dy_fitter$par[1]),origin = c(dx_fitter$par[2]-100*dx_fitter$par[1],dy_fitter$par[2]-100*dy_fitter$par[1]),ncols = 400,nrows = 400), asp=1, pch=3, col="Blue")
    
        # find and remove_worst_fitting_points
    better_pts = remove_worst_fitting_points(good_pts,f_chi=function(pts) { return (
      modified_chi2_lines(dx_fitter$par, pi, pts)
      + modified_chi2_lines(dy_fitter$par, 0, pts)
    ) },relative_share_to_remove = 0.1)
    dy_fitter=sbgm(good_pts, seq(5.8,6.6,0.1), seq(0,7,1), 0)
    dx_fitter=sbgm(good_pts, seq(5.8,6.0,0.1), seq(0,7,1), pi/2)
    # get the global offset by checking the minimal bins where the points fall into
    better_pts[,bin_x:=floor((x-dx_fitter$par[2])/dx_fitter$par[1])]
    better_pts[,bin_y:=floor((y-dy_fitter$par[2])/dy_fitter$par[1])]
    better_pts[,bin_x_count:=.N,by="bin_x"]
    better_pts[,bin_y_count:=.N,by="bin_y"]
    offset_y = min(better_pts$bin_x) * dx_fitter$par[1] + dx_fitter$par[2]
    offset_x = min(better_pts$bin_y) * dy_fitter$par[1] + dy_fitter$par[2]
    grid_size_x = max(better_pts$bin_x)-min(better_pts$bin_x)
    grid_size_y = max(better_pts$bin_y)-min(better_pts$bin_y)
    # grid_size_x
    # grid_size_y
    
    fitted_grid=create_grid(angles = c(0, 90),length = c(dx_fitter$par[1],dy_fitter$par[1]),origin = c(offset_x, offset_y), ncols = grid_size_y, nrows = grid_size_x)
    ...
    feature_spots[, new_y:= if (x_id %% 2==0) y_id=rev(seq(1,63,by=2)), by = x_id]
    feature_spots[, new_y:= if (x_id %% 2!=0) y_id=rev(seq(2,62,by=2)), by = x_id]
    
    feature_spots[,bc:=paste0(x_id,"x",new_y),]

    feature_spots[,feature_spot:=!is.na(value),]
    feature_spots[,value:=NULL,]
    
    feature_spots[,x_prec:=x,]
    feature_spots[,y_prec:=y,]
